...
export default class Game extends Phaser.Scene {
    ...
    spawnCoin(tile) {
      if (Phaser.Math.Between(0, 11) > 5) {
        this.time.delayedCall(500, () => { this.coins.add(new Coin(this, tile.pixelX + 16, tile.pixelY + 16))}, null, this);
      }
    }
    ...
    showLogo () {
      if (!this.showingLogo) {
        ...
        new Bat(this,logoPosition.x + 800, logoPosition.y - 20, "left")
        new Bat(this,logoPosition.x - 600, logoPosition.y - 100, "right")
        new Bat(this,logoPosition.x + 600, logoPosition.y - 220, "left")
        new Bat(this,logoPosition.x + 400, logoPosition.y + 20, "right")

        this.hideScore();
        this.cameras.main.stopFollow();
        this.cameras.main.pan(logoPosition.x + 330, logoPosition.y + 150, 2000, 'Sine.easeInOut')
        //this.showTitle(logoPosition.x, logoPosition.y);
      }
    }

    hideScore() {
      this.scoreCoins.destroy();
      this.scoreCoinsLogo.destroy();
    }
        ...
        flags.forEach((flag, i) => {
            this.time.delayedCall(500 * (i+1) + 800,
                () => {
                    this.playAudioRandomly("stone_fail")
                    this.playAudioRandomly("stone")
                    let image = this.add.image((x + 180) + (i * 125), y + 150, flag).setOrigin(0.5)
                    this.tweens.add({
                      targets: image,
                      duration: 300,
                      y: "+=10",
